...
void destroyGrid();				//!<	Освобождение памяти, отведенной для хранения сетки.
void initData();				//!<	Выделение памяти и задание нач. условий.
void destroyData();				//!<	Освобождение памяти.
void calcIntegralSurf();		//!<	Вычисление поверхностных интегралов в невязке.
void calcIntegralVol();			//!<	Вычисление объемных интегралов в невязке.
void calcNewValues();			//!<	Вычисление значений на след. временном шаге.
void calcResidual();			//!<	Вычисление невязок
...
int main()
{
	...
	for (int i = 0; i < A_block_size * cellsCount; i++) {
		b_data[i] = 0.0;
		lastLayerSolution[i] = 0.0;
		currentLayerSolution[i] = 0.0;
	}
	...
	print_right_part("lastLayerSolution_0.txt", lastLayerSolution);
	initRightPart(lastLayerSolution, b_data);
	print_right_part("right_part_0.txt", b_data);
	...
	//int nnz_amgx = A_small_block_size * A_small_block_size * A_block_size * nnz;
	int n_amgx = matrix.n;
...
void initData()
{
	...
	funcCount = 3;
	edgeGPCount = 2;// Мы будем считать только для 2
	cellGPCount = 3;// Мы будем считать только для 3
	...
	for (int i = 0; i < cellsCount; i++)
	{
		...
		//if (cellNeigh[i][j] == -1)		//если ячейка является граничной
		//{
		//isBoundary = true;
		//}
		//}

		//if (isBoundary)	//в каждой ячейке
		//{
...
// *********************************************** Данные для квадратур Гаусса **********************************************************************
void calcCellGP(int iCell)	//Сделал для 3ёх точек.
{
	float a = 1.0 / 6.0;
	float b = 2.0 / 3.0;
...
void calcGradientsSurfIntegral()
{
	...
	for (int i = 0; i < edgesCount; i++)
	{
		...
		c2 = edgeCell2[i];	// внешняя ячейка для i - ого ребра
		if (c2 >= 0) {
			isBoundary = false;

			for (int iF = 0; iF < funcCount; iF++) // каждый случай при скалярном умножении уравнений на baseF( iF, ... )
			{
				float tmpIntWx1 = 0.0, tmpIntWy1 = 0.0; // для ячейки С1
				float tmpIntWx2 = 0.0, tmpIntWy2 = 0.0; // для ячейки С2
				for (int iGP = 0; iGP < edgeGPCount; iGP++)
				{
					...
					// Оставил по одному интегралу при Kxx и Kyy
...
void calcIntegralSurf()	// —читаем поверхностный интеграл
{
	float FU, FWx, FWy;	// потоки на границе ячейки
							//float FUl, FWxl, FWyl;	// потоки на границе "-"
							//float FUr, FWxr, FWyr;  // потоки на границе "+"
	float Ul = 0.0, Wxl = 0.0, Wyl = 0.0;  // прим. переменные в соседней ячейке
	float Ur = 0.0, Wxr = 0.0, Wyr = 0.0;  // прим. переменные в соседней ячейке
	...
	for (int i = 0; i < edgesCount; i++)
	{
		c1 = edgeCell1[i];	// внутрянняя ячейка для i - ого ребра
		c2 = edgeCell2[i];// внешняя ячейка для i - ого ребра
		if (c2 >= 0) {
			isBoundary = false;
			...
			for (int iF = 0; iF < funcCount; iF++) // каждый случай при скалярном умножении уравнений на baseF( iF, ... )
			{
				float tmpIntU1 = 0.0, tmpIntWx1 = 0.0, tmpIntWy1 = 0.0; // для ячейки С1
				float tmpIntU2 = 0.0, tmpIntWx2 = 0.0, tmpIntWy2 = 0.0; // для ячейки С2
				for (int iGP = 0; iGP < edgeGPCount; iGP++)
				{
					...
					// интеграла по потоку FU должно быть два - nx, ny
			...
			for (int iF = 0; iF < funcCount; iF++) // каждый случай при скалярном умножении уравнений на baseF( iF, ... )
			{
				float tmpIntU1 = 0.0; //для ячейки С1
				for (int iGP = 0; iGP < edgeGPCount; iGP++)
				{
					getFields(Ul, Wxl, Wyl, c1, edgeGP[i][iGP].x, edgeGP[i][iGP].y, NEW);
...
// ******************************************* Получение вспомогательных значений *******************************************************************
void getFlux(float& FU, float& FWx, float& FWy,
	float Ul, float Wxl, float Wyl,
	float Ur, float Wxr, float Wyr, Vector n, bool isBoundary) // Получаем потоковые значения.
{
	float c11 = 1.0;
...
void initRightPart(float* lastLayerSolution, float* rightPart_data)
{
	...
	for (int i = 0; i < A_size; i++) {
		...
		// rowId in each small block - пробная функция
		...
		if (c2 >= 0) {
			...
			for (int m = 0; m < funcCount; m++)
			{
				...
				for (int j = 0; j < A_small_block_size; j++)
				{
					...
					float tmpC1C2_y = 0.0, tmpC2C1_y = 0.0;

					for (int iGP = 0; iGP < edgeGPCount; iGP++)
					{
						float cGP1_1 = edgeWGP[iGP] * baseF(m, c1, edgeGP[i][iGP].x, edgeGP[i][iGP].y) * baseF(j, c1, edgeGP[i][iGP].x, edgeGP[i][iGP].y);
						float cGP1_2 = edgeWGP[iGP] * baseF(m, c1, edgeGP[i][iGP].x, edgeGP[i][iGP].y) * baseF(j, c2, edgeGP[i][iGP].x, edgeGP[i][iGP].y);
					...
					if (edgeNormal[i].x * V_x + edgeNormal[i].y * V_y >= 0.0) {
						iCell = c1;
						iRowCurrent = iRowU1Current;
					}
					else {
						iCell = c2;
						iRowCurrent = iRowU2Current;
					}
					

					for (int iGP = 0; iGP < edgeGPCount; iGP++)
					{
						float cGP1 = edgeWGP[iGP] * baseF(m, c1, edgeGP[i][iGP].x, edgeGP[i][iGP].y) * baseF(j, iCell, edgeGP[i][iGP].x, edgeGP[i][iGP].y);
						float cGP2 = edgeWGP[iGP] * baseF(m, c2, edgeGP[i][iGP].x, edgeGP[i][iGP].y) * baseF(j, iCell, edgeGP[i][iGP].x, edgeGP[i][iGP].y);

						tmpC1 += (V_x * edgeNormal[i].x * cGP1 + V_y * edgeNormal[i].y * cGP1);
						tmpC2 += (V_x * edgeNormal[i].x * cGP2 + V_y * edgeNormal[i].y * cGP2);
					}

					rightPart_data[iRowU1Current] -= edgeJ[i] * lastLayerSolution[iRowCurrent] * tmpC1;
					rightPart_data[iRowU2Current] += edgeJ[i] * lastLayerSolution[iRowCurrent] * tmpC2;
			...
			for (int m = 0; m < funcCount; m++)
			{
				for (int j = 0; j < A_small_block_size; j++)
				{
					...
					if (edgeNormal[i].x * V_x + edgeNormal[i].y * V_y >= 0.0) {
						...
						tmpIntU1 *= edgeJ[i] * lastLayerSolution[iRowU1Current];
		...
		if (c2 >= 0) {
			...
			for (int m = 0; m < funcCount; m++)
			{
				...
				for (int j = 0; j < A_small_block_size; j++)
				{
					...
					//for u_qx
					addEdgeIntForBlockWithoutTau(A, iRowU1Current, iColQx1 + j, -tmpC1C1_x, i);
					addEdgeIntForBlockWithoutTau(A, iRowU1Current, iColQx2 + j, -tmpC1C2_x, i);
					addEdgeIntForBlockWithoutTau(A, iRowU2Current, iColQx2 + j, tmpC2C2_x, i);
					addEdgeIntForBlockWithoutTau(A, iRowU2Current, iColQx1 + j, tmpC2C1_x, i);

					//for u_qy
					addEdgeIntForBlockWithoutTau(A, iRowU1Current, iColQy1 + j, -tmpC1C1_y, i);
					addEdgeIntForBlockWithoutTau(A, iRowU1Current, iColQy2 + j, -tmpC1C2_y, i);
					addEdgeIntForBlockWithoutTau(A, iRowU2Current, iColQy2 + j, tmpC2C2_y, i);
					addEdgeIntForBlockWithoutTau(A, iRowU2Current, iColQy1 + j, tmpC2C1_y, i);
...
void print_right_part(const char* fileName, float* rightPart_data) {
	...
	for (int i = 0; i < A_block_size * cellsCount; i++) {
		fprintf(fp, "%10.8f ", rightPart_data[i]);
		fprintf(fp, "\n");
	}
}
